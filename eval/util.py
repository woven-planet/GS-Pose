...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    ...
    plt.ylabel('AP%',fontsize=fs)
    plt.ylim((0, 1))
    plt.xlabel('3D IoU thresholds',fontsize=fs)
    ...
    iou_aps = iou_3d_aps
    for cls_id in range(1, num_classes):
        #print('{} 3D IoU at 10: {:.1f}'.format(synset_names[cls_id], iou_aps[cls_id, iou_thres_list.index(0.1)] * 100))
        ...
        print('{} 3D IoU at 75: {:.1f}'.format(synset_names[cls_id], iou_aps[cls_id, iou_thres_list.index(0.75)] * 100))
